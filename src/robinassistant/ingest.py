...
import re
import datetime
import src.robinassistant.utils.finance_utils as f_utils


class ETL:
    def __init__(self):
        self.paths = {"token": "../data/token.txt", "data": "../data/stocks.txt"}
    ...
    def additional_robin_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["Total Return"] = (
            df["Shares"] * df["Price"] - df["Shares"] * df["Average Cost"]
        )
    ...
    def add_prices(self):
        """Combines current data with actual market API data"""
        start_date = datetime.datetime.today() - datetime.timedelta(2)

        # Calculating the most recent price
        prices = f_utils.price_action(
            df.Symbol, token_path=self.paths["token"], start_date=start_date
        )
